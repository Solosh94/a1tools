...
/// Allows managing user access to specific groups within a board
library;
...
class _GroupMembersDialogState extends State<GroupMembersDialog> {
  ...
  Future<void> _loadMembers() async {
    ...
    final members = await SundayService.getGroupMembers(widget.groupId);
    if (mounted) {
      setState(() {
        _members = members;
        _loading = false;
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${widget.groupTitle} • ${widget.boardName}',
                          style: TextStyle(
                            fontSize: 12,
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Users assigned here can only see this group and its items within the board.',
                      style: TextStyle(
                        fontSize: 12,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'Add users to give them access to only this group',
            style: TextStyle(
              fontSize: 12,
  ...
  Widget _buildMemberTile(GroupMember member, bool isDark) {
    return Card(
  ...
  void _showAddMemberDialog() {
    showDialog(
      context: context,
      builder: (context) => _AddGroupMemberDialog(
  ...
  Future<void> _addMember(String username, String accessLevel) async {
    final success = await SundayService.addGroupMember(
    ...
    if (success) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('User added to group'),
  ...
  Future<void> _updateMemberAccess(GroupMember member, String accessLevel) async {
    final success = await SundayService.updateGroupMemberAccess(
      memberId: member.id,
      accessLevel: accessLevel,
  ...
  Future<void> _removeMember(GroupMember member) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Remove ${member.displayName}\'s access to this group?',
        ),
        actions: [
          TextButton(
    ...
    if (confirm == true) {
      final success = await SundayService.removeGroupMember(
        member.id,
        widget.username,
      );
...
/// Dialog for adding a new member to the group
